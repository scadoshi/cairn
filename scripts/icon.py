#!/usr/bin/env python3
"""Render the app icon set from the wordmark's "o".

    scripts/icon.py

Reads assets/odo.txt, cuts the first letter out (8 cells wide, leaning one
column per row), writes it to assets/o.txt, and renders it with JetBrains
...
def mark_from_wordmark() -> list[str]:
    rows = (ROOT / "assets/odo.txt").read_text().rstrip("\n").split("\n")
    band = [r[: i + 8] for i, r in enumerate(rows) if r[: i + 8].strip()]
    indent = min(len(t) - len(t.lstrip(" ")) for t in band)
    mark = [t[indent:] for t in band]
    (ROOT / "assets/o.txt").write_text("\n".join(mark) + "\n")
    return mark
...
def ttf_path() -> str:
    ...
    font.flavor = None
    out = pathlib.Path(tempfile.gettempdir()) / "odo-jbm-400.ttf"
    font.save(out)
    return str(out)
