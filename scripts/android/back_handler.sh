...
# both the gesture and the button and forwards it to the app as an `crow:back`
...
KT="${1:-$REPO_ROOT/target/dx/crow/release/android/app/app/src/main/kotlin/dev/dioxus/main/MainActivity.kt}"
        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                appWebView?.evaluateJavascript(
                    "window.dispatchEvent(new Event('crow:back'))",
                    null
                )
            }
        })
